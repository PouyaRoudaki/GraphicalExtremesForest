...
finalResultConnected<-vector()
counter <- 0


finalResultConnected <- foreach (i = 31:length(nSeq),
                        .combine = rbind,
...
finalResultConnectedDF <- data.frame(finalResultConnected)
colnames(finalResultConnectedDF) <- c("n","k","p",
                           "simulation",
                           as.character(c(1:length(empChiMat[connectedInd]))))

saveRDS(finalResultConnected, file="finalResultConnected.RData")
saveRDS(finalResultConnectedDF, file="finalResultConnectedDF.RData")
