...
forestSimulationHR <- function(forestStructureList, n, p=0, k=NULL){
  ...
  empVarioMatDirectVario <- emp_vario(simulatedData, k=NULL, p) 
  empVarioMatChi2Vario <- chi2Gamma(empChiMat)
  #'print(empVarioMat)                               
  ...
  #' store resultant MSF in the dataframe format each row: each edge's vertices 
  #' with its weight
  empDirectVarioGraphDf <- data.frame( FirstVertice = numeric(),  
                                    SecondVertice = numeric(), 
                                    weight = numeric(), 
                                    stringsAsFactors=FALSE) 

  empChi2VarioGraphDf <- data.frame( FirstVertice = numeric(),  
                                    SecondVertice = numeric(), 
                                    weight = numeric(), 
                                    stringsAsFactors=FALSE) 
  ...
  for (i in (1:(d-1))) {                                           
    for (j in ((i+1):d)) {
      ...
      empChiGraphDf[e,1] <- i
      empDirectVarioGraphDf[e,1] <- i
      empChi2VarioGraphDf[e,1] <- i

      empChiGraphDf[e,2] <- j
      empDirectVarioGraphDf[e,2] <- j
      empChi2VarioGraphDf[e,2] <- j

      empChiGraphDf[e,3] <- empChiMat[i,j]
      empDirectVarioGraphDf[e,3] <- empVarioMatDirectVario[i,j]  
      empChi2VarioGraphDf[e,3] <- empVarioMatChi2Vario[i,j]

      e <- e + 1
    }
  }
  
  empiricalEstGraphDfList <- list(empChiGraphDf,empDirectVarioGraphDf,empChi2VarioGraphDf)
